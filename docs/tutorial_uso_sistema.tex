...
\begin{enumerate}[label=\arabic*)]
    ...
    \item \textbf{Integración ASPEN HYSYS}: Validación cruzada con simulador comercial
...
\begin{itemize}
    \item ASPEN HYSYS (solo Windows, para módulo de integración)
...
pywin32 & Integración con ASPEN HYSYS (solo Windows) \\
\bottomrule
\end{tabular}
\end{table}
...
cp plantillas/plantilla_config.yaml config.yaml
    ...
    \begin{itemize}
        ...
        \item \texttt{hysys\_integration}: Habilitar/deshabilitar HYSYS
...
\subsection{Módulo 4: Simulación en ASPEN HYSYS (Opcional)}

\subsubsection{Requisitos Previos}

\begin{warningbox}{Solo Windows}
Este módulo requiere:
\begin{itemize}
    \item Sistema operativo Windows
    \item ASPEN HYSYS instalado
    \item Licencia válida de HYSYS
    \item Biblioteca pywin32 instalada (\texttt{pip install pywin32})
\end{itemize}

Si no tienes HYSYS, puedes omitir esta sección. El sistema standalone es completamente funcional.
\end{warningbox}

\subsubsection{Objetivo}

Validar el modelo cinético standalone usando el simulador comercial ASPEN HYSYS.

\subsubsection{Ejecución}

\begin{lstlisting}[style=bashstyle]
python main.py --mode simulate_hysys \
    --input results/parameter_fitting/parametros_ajustados.json \
    --output results/hysys/
\end{lstlisting}

O usar el script de ejemplo:

\begin{lstlisting}[style=bashstyle]
cp plantillas/ejemplo_04_simular_hysys.py .
python ejemplo_04_simular_hysys.py
\end{lstlisting}

\subsubsection{¿Qué hace el módulo?}

\begin{enumerate}
    \item Inicia ASPEN HYSYS automáticamente
    \item Crea un nuevo caso (.hsc)
    \item Configura componentes: Metanol, Tripalmitin, MethylPalmitate, Glycerol
    \item Selecciona paquete termodinámico UNIFAC
    \item Crea reactor CSTR con volumen = 20 L
    \item Configura cinética Arrhenius con parámetros ajustados
    \item Convierte datos batch → continuo
    \item Ejecuta simulación
    \item Extrae resultados (conversión, composición)
    \item Guarda caso HYSYS
    \item Cierra HYSYS
\end{enumerate}
...
\begin{itemize}
    \item \texttt{results/hysys/resultados\_hysys.json}: Conversión y composición
    \item \texttt{results/hysys/caso\_biodiesel.hsc}: Caso HYSYS guardado
\end{itemize}

\subsection{Módulo 5: Comparación de Modelos}

\subsubsection{Objetivo}

Comparar estadísticamente los resultados del modelo standalone vs ASPEN HYSYS.
...
Si R$^2 > 0.95$ y MAPE < 5\%, tu modelo standalone está validado exitosamente contra HYSYS. Puedes confiar en los resultados del modelo para predicciones y optimización.
\end{successbox}
...
\subsubsection{Parity Plot (Standalone vs HYSYS)}

\textbf{Ubicación}: \texttt{results/comparison/parity\_plot.png}

Gráfica de valores predichos por standalone vs HYSYS.
...
\textbf{Solución}: Es normal. El módulo de HYSYS simplemente no estará disponible. El resto del sistema funciona perfectamente.
...
\subsection{Errores de ASPEN HYSYS}

\subsubsection{Error: "HYSYS.Application not found"}

\textbf{Causa}: HYSYS no está instalado o no registrado en COM.

\textbf{Solución}:
\begin{enumerate}
    \item Verificar que HYSYS esté instalado
    \item Ejecutar HYSYS manualmente al menos una vez
    \item Re-registrar COM: ejecutar HYSYS como administrador
\end{enumerate}

\subsubsection{Error: "Access denied" al conectar con HYSYS}

\textbf{Causa}: Permisos insuficientes.

\textbf{Solución}: Ejecutar Python/script como administrador.

\subsubsection{Simulación HYSYS no converge}

\textbf{Causa}: Condiciones termodinámicas inválidas o parámetros extremos.

\textbf{Solución}:
\begin{enumerate}
    \item Verificar propiedades de componentes en HYSYS
...
Simular en HYSYS & \texttt{python main.py --mode simulate\_hysys} \\
...
\begin{description}
    ...
    \item[HYSYS] Software de simulación de procesos de Aspen Technology
